...
describe('useScopedPermissions', () => {
    ...
    it('should fetch scoped permissions on mount', async () => {
        ...
        expect(mockBrowser.runtime.sendMessage).toHaveBeenCalledWith({ type: 'GET_PERMISSION_FOR_TAB' });
    });
    ...
    it('should request scoped permission', async () => {
        ...
        expect(mockBrowser.permissions.request).toHaveBeenCalledWith({ origins: ['*://*.example.com/*'] });
